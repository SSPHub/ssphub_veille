...
from src.data.to_infolettre import concatenate_infolettre, create_veille_qmd
from src.git.clone import (
    add_commit,
    clone_branch,
    extract_branch_infolettre,
    list_remote_branches,
    max_infolettre,
)
from src.utils.access_grist_api import GristApi
from src.utils.config import COL_CATEGORY, TABLE_RUBRIQUES
...
def extract_rows_qmd(
    ...
    veille_df = api.fetch_table_pl(table_id=input_table)
    logger.info(
    ...
    veille_df = veille_df.filter(pl.col("A_garder"), pl.col("Lien_veille") == "")
    ...
    veille_df = (
        veille_df.explode(COL_CATEGORY)
        .filter(pl.col(COL_CATEGORY) != "L")  # droping "L"
        ...
        .join(
            veille_df.drop(COL_CATEGORY), how="right", on="id", maintain_order="right"
        )
    )
...
def append_commit(
    output_path="veille.qmd", repo="https://github.com/inseefrlab/ssphub"
):

    folder_repo = repo.split("/")[-1]
    _, infolettre_branch = max_infolettre(
