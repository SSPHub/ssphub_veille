import polars as pl
from src.utils.access_grist_api import (
    get_grist_api,
    download_table
    )
...
def add_to_veille(my_conv_df, target_table='Test', logger=setup_logging()):
    ...
    logger.info(f"Nombre d'enregistrements ajoutés dans la table {target_table} : {len(res)}")
...
def extract_and_add_to_veille(input_conv_file_path='ssphub_veille/export.json', target_table='Test', logger=setup_logging()):
    """
